...
void HMC5883_Init(struct HMC5883 *hmc)
{        
	buffer[0] = 0x00;
	buffer[1] = 0x78;       // Sampling Number, Output Rate
	HAL_I2C_Master_Transmit(hmc->hi2c, hmc->address, buffer, 2, 10);
...
void HMC5883_Read(struct HMC5883 *hmc)
{        
        buffer[0] = 0x03;
        HAL_I2C_Master_Transmit(hmc->hi2c, hmc->address, buffer, 1, 10);
        HAL_I2C_Master_Receive(hmc->hi2c, hmc->address, &buffer[1], 6, 10);
