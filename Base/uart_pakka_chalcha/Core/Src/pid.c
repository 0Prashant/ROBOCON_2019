...
extern float data_from_navigator[3];
...
void pid_distance()
{
	distance_pid[0] = compute_pid_distance(&pid_dis_1,required_dx,data_from_navigator[0]);
	distance_pid[1] = compute_pid_distance(&pid_dis_2,required_dy,roboty);
	distance_pid[2] = compute_pid_distance(&pid_dis_3,required_theta,data_from_navigator[2]);
