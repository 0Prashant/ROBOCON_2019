...
*/
	...
	time = distance/ROBOT_VELOCITY;
	calculate_robot_distance();	
	while(((float_abs(del_distance_pid[0])>0) || (float_abs(del_distance_pid[1])>0) || (float_abs(del_distance_pid[2])>0))) 
	//while (pow((pow((previous_robotx-robotx),2)+pow((previous_roboty-roboty),2)) , 0.5) < distance)
	{
		pid_distance();
