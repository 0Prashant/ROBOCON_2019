...
static Queue<int16_t, 32> gWheeler_Data[NUM_WHEELER_BYTES / 2];
...
static float calculate_Omega(float velocity)
{
        return (velocity) / (float)(WHEEL_RADIUS);
}
...
void send_WheelerPack(int16_t pack[NUM_WHEELER_BYTES / 2])
{
        // Data are omegas of two motors
        for (uint8_t i = 0; i < NUM_WHEELER_BYTES / 2; ++i) {
                gWheeler_Data[i].insert(pack[i]);
        }
