...
bool go(int steps, float angle)
{
	...
	if (fabs(angle) != fabs(robot_angle))
	{
		if (leg[0].is_raised())
		{
			...
			{
				if (angle > (robot_angle + tolerence))
				{
					steering.set_omega(steering_speed);
					printf("+++");
				}
				else if (angle < (robot_angle - tolerence))
				{
					steering.set_omega(-steering_speed);
					printf("---");
				}
		...
		{
			if (steering.get_angle() > (tolerence))
			{
				steering.set_omega(-steering_speed);
			}
			else if (steering.get_angle() < (-tolerence))
			{
				steering.set_omega(steering_speed);
			}
			else
	...
	printf("\nrobot_angle = %d \t steering_angle = %d", (int)(robot_angle*100), (int)(steering.get_angle()*100));
