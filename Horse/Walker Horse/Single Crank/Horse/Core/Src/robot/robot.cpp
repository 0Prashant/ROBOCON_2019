...
static const float robot_speed = 15;
static float leg_speed = robot_speed;    //35 is the maximum with safe zone
static const float steering_speed = 1.25; // 1.35 is the 100%
...
float steps[7] = {10, 17, 23, 27, 34, 45, 70};
...
bool go(int steps, float angle)
{
	...
	printf("steps = %d \trobot_angle = %d\t steering_angle = %d\t leg_angle = %d\n",
	leg[0].steps,  (int)(robot_angle*180/PI), (int)(steering.get_angle()*180/PI), (int)(leg[0].get_angle()*180/PI));
	
	if ((leg[0].get_steps() >= steps) && (fabs(angle - robot_angle) <= 0.1))
...
void calculate_datas()
{
	...
	printf("\tangleZ = %d\t", (int)(angle.getZ()));
	// printf("\tangleZ = %d \t robotangle = %d \t leg_omega = %d \t steering_omega = %d\n",
	//   (int)(angle.getZ()), (int)(robot_angle*180/PI), (int)(leg[0].get_omega()*100), (int)(steering.get_angle()*100));
}
