...
extern int16_t gCounter;
extern uint16_t bomega;
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_PIN)
{
        ...
        if (GPIO_PIN == GPIO_PIN_0) //blnc_motor_chA_Pin == D0
        {
                if (HAL_GPIO_ReadPin(GPIOD, GPIO_PIN_2)) //blnc_motor_chB_Port-pin == D2
                {
			++gCounter;
                }
                else
