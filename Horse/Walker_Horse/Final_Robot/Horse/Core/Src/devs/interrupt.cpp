...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
		...
		case GPIO_PIN_0 : {
			...
			float tolerence = 20*PI/180;
			if((leg[0].get_angle()>(leg0_reset_angle-tolerence)) && (leg[0].get_angle()<(leg0_reset_angle+tolerence))){
			...
			float tolerence = 20*PI/180;
			if((leg[1].get_angle()>(leg1_reset_angle-tolerence)) && (leg[1].get_angle()<(leg1_reset_angle+tolerence))){
