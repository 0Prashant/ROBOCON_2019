...
static void pid_init(){
	...
	steering.set_PID_constants( 2.3	, 50, 0, -8.75, 8.75);		//2.3, 50
	steering.set_angle_PID_constants( 0.164, 0.0002, 0, -2.4, 2.4);	//0.164, 0.0002
}
...
void motor_init(){
        ...
        motor_configurations[0].in1_port = GPIOA;
        motor_configurations[0].in1_pin = GPIO_PIN_10; 
        motor_configurations[0].in2_port = GPIOA;
        motor_configurations[0].in2_pin = GPIO_PIN_8;
        motor_configurations[0].channel = TIM_CHANNEL_4;
	...
	HAL_TIM_PWM_Start(motor_configurations[0].htim, TIM_CHANNEL_2);
        HAL_TIM_PWM_Start(motor_configurations[1].htim, TIM_CHANNEL_4);
        HAL_TIM_PWM_Start(motor_configurations[2].htim, TIM_CHANNEL_3);
}
