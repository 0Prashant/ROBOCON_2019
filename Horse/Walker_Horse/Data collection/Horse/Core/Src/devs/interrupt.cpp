...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
		...
		case GPIO_PIN_0 : {
			...
			static int count1 = 0;
			if(((leg[0].get_angle()>(leg0_reset_angle-tolerence)) && (leg[0].get_angle()<(leg0_reset_angle+tolerence))) || (count1>0)){
			...
			float tolerence = 40*PI/180;
			static int count2 = 0;
			if(((leg[1].get_angle()>(leg1_reset_angle-tolerence)) && (leg[1].get_angle()<(leg1_reset_angle+tolerence))) || (count2>0)){
		...
		case GPIO_PIN_13: {
			STEERING_FLAG = true;
			float tolerence = 3.5*PI/180;
			static int count3 = 0;
			if(((steering.get_angle()>(-tolerence)) && (leg[1].get_angle()<(tolerence))) || (count3>0)){
