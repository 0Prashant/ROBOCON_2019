...
static const float steering_speed = 0.73; // 0.875 is the 100%
static const float steering_angle_limit = 8.4 * PI / 180;
...
void move_leg(int step, float angle)
{
	...
	leg_speed = (leg[0].is_raised_without_deadzone()) ? robot_speed : robot_speed+1;
	//Setting the slow speed in sand dune and tussok
	leg_speed = ((step==100)||(step== (int)steps[6])) ? 4.3 : leg_speed;
